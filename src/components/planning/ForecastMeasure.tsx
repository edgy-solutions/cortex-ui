...
import { ACCENT, OVER } from "@/lib/chartPalette";
  ...
  return (
    <div className="min-w-0">
      ...
      <p
        ...
        style={{ color: known && tone === "over" ? OVER : undefined }}
      >
        {known ? formatAmount(value, unit) : <span className="text-slate-600">—</span>}
      </p>
    </div>
  );
}
    ...
    <div className="glass-panel p-4">
      ...
      <div className="mt-3 flex items-end gap-4 flex-wrap">
        <p
          ...
          style={{ color: overBudget ? OVER : ACCENT }}
        >
          {formatAmount(row.eac, value_unit)}
        </p>
        <div className="min-w-0">
          <p className="font-mono text-[11px] text-slate-200 uppercase tracking-widest">
            {row.method}
          </p>
      ...
      <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Quantity label="budget" value={row.bac} unit={value_unit} />
        <Quantity
          label="variance"
          value={row.vac}
          unit={value_unit}
          tone={overBudget ? "over" : "plain"}
        />
        <Quantity label="to complete" value={row.etc} unit={value_unit} />
        <Quantity label="spent" value={row.acwp} unit={value_unit} />
      </div>

      <div className="mt-3 flex items-center gap-4 flex-wrap font-mono text-[10px] text-slate-400">
        {typeof row.cpi === "number" && <span>CPI {showMeasure(row.cpi, 2)}</span>}
        {typeof row.spi === "number" && <span>SPI {showMeasure(row.spi, 2)}</span>}
